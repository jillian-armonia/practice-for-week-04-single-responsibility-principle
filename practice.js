...
function bakeAndSellPies(pieType, pieQuantity, profitMargin = 1.2) {
  ...
  //Calculate the cost of each pie
  let costOfPie = calculateCost(recipe);
...
function bakePies(recipe, pieType, pieQuantity){
  ...
  for (let i = 0; i < pieQuantity; i++) {
    ...
    let ingredients = recipe.map(ingredient => ingredient.name).join(', ');
    console.log(combiningMsg + ingredients);
